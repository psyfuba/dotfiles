...
local create_button_time = function (color, bg_color, hover_color)
	local widget = wibox.widget {
		...
		valign = "left",
		--markup = helpers.colorize_text(symbol, color),
		widget = wibox.widget.textclock(helpers.colorize_text("%I %M %p", color))
	}
...
local time_button = create_button_time(x.color1, x.color0, x.color0)
--time_button:buttons(gears.table.join(
    --awful.button({ }, 1, function ()
    --    app_drawer_show()
    --end),
